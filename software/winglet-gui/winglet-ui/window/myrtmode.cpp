...
MyrtMode::MyrtMode(QWidget *parent)
    : QWidget{parent}
{
    // MYRT is a dedicated map-backed radar mode.
    viewMap = true;
    sdMap = true;
    mapLevel = 10;
    pixel_to_miles = mapValue[mapLevel];
    ...
    QObject::connect(zoomDebounceTimer, &QTimer::timeout, this, &MyrtMode::processDebouncedZoom);
//    this->resize(480, 480);
...
void MyrtMode::hideEvent(QHideEvent *ev) {
    (void) ev;
}

void MyrtMode::wheelEvent(QWheelEvent *wheelEvent) {
...
void MyrtMode::keyPressEvent(QKeyEvent *ev) {
...
void MyrtMode::paintEvent(QPaintEvent *pEvent)
{
    (void) pEvent;
    ...
    // MYRT: draw the rotating radar sweep over the map.
    // The physical LED sweep is still driven by rotate().
    QPen sweepPen(*activeTheme->radar_sweep_lines_color);
    sweepPen.setWidth(2);
...
QPoint MyrtMode::rotate_matrix(double x, double y, double x_shift, double y_shift, double angle){
    if(x_shift == 0){
        x_shift = 240;
    }
    if(y_shift == 0){
...
void MyrtMode::drawCirle(QPainter *paint, int val){
...
void MyrtMode::rotate(double increment ){
...
void MyrtMode::rotate_line_matrix(double x, double y, double angle ){
    // shift to origin (0,0)
    angle = angle * (M_PI/180);
    double s = sin(angle);
    double c = cos(angle);
...
void MyrtMode::drawPlane(QPainter *paint, QPoint coord, float distance, Aircraft *entry, bool showInfo){
    if (entry->latValid && entry->lonValid){
...
void MyrtMode::changeMap(){
   viewMap = true;
}


void MyrtMode::changeMapLocation()
{
    sdMap = !sdMap;
}
...
bool MyrtMode::updateMap(bool doForce){
...
bool MyrtMode::setMapTile(double zoom, double lat, double lon){
...
void MyrtMode::processDebouncedZoom()
{
